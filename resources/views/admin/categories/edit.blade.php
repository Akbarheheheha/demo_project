...
<div class="space-y-6 max-w-xl">
    ...
    <div class="flex items-center gap-3">
                <a href="{{ route($rolePrefix . '.categories.index') }}" 
        ...
        <div>
            ...
            <p class="text-sm text-slate-500">Perbarui nama kategori produk: {{ $category->name }}.</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <form action="{{ route($rolePrefix . '.categories.update', $category->id) }}" method="POST" class="space-y-6">
            @method('PUT')
