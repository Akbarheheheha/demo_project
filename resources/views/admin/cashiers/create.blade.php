...
<div class="space-y-6 max-w-4xl">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Tambah Akun Kasir Baru</h2>
            <p class="text-sm text-slate-500">Daftarkan akun kasir baru agar dapat login ke dashboard POS.</p>
        </div>
    </div>

    <!-- Form Container -->
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <form action="{{ route($rolePrefix . '.cashiers.store') }}" method="POST" class="space-y-6">
            <!-- Include the shared form partial -->
            @include('admin.cashiers.form')
